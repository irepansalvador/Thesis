...
In addition to the external shape of the embryo, the shape complexity of its internal structures (e.g., organs) is expected to increase during development \citep{Sharpe2003}. In order to analyse the shape of the internal parts of an embryo, staining and imaging methods are usually required. One useful approach is the use of techniques that highlight the distribution of a tissue-specific gene products (e.g., whole-mount in situ mRNA hybridization or immuno-histochemistry techniques).
Therefore, the shape of gene expression patterns (represented in 2D or 3D) can be used to describe the complexity of the embryo. When analyzing the shape of a gene expression through development, it could be that the expression pattern: 1) does not change its complexity, 2) becomes more complex, or 3) becomes less complex.
However, it would be expected that when analysing a large number of genes, the shape complexity would increase at a statistical level.
%
%also be expected to increase its spatial complexity during development. 

In order to analyse the complexity of the shape of an object is necessary to quantitatively measure and analyse it. The study of morphometrics refers to the quantitative analysis of morphological shape. In the last decades, morphometric tools have been used widely as a tool to quantify, characterize and compare biological shapes \citep{JamesRohlf1993}. In the next paragraphs I will briefly explain the most important morphometric methods. For an extensive review, see \citep{Bookstein1997,Dryden1998,Zelditch2008,Slice2005}.
